...
function* tokenListSearchFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.tokenListSearchFulfilled,
                payload: {
                    data: []
                }
            });
        ...
        yield put({
            type: AT.tokenListSearchRejected
        });
    }
}
...
function* tokenSummaryFunc(action) {
    try {
        ...
        yield put({
            type: AT.tokenSummaryRejected,
            error: action.payload.contractAddr
        });
    }
}
...
function* tokenTransfersListFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.tokenTransfersListFulfilled,
                payload: {
                    data: []
                }
            });
        ...
        yield put({
            type: AT.tokenTransfersListRejected
        });
    }
}
...
function* tokenHoldersListFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.tokenHoldersListFulfilled,
                payload: {
                    data: []
                }
            });
        ...
        yield put({
            type: AT.tokenHoldersListRejected
        });
    }
}
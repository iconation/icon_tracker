...
export function* addressDelegationListFunc(action) {
    try {
        ...
        yield put({
            type: AT.addressDelegationListRejected
        });
    }
}
...
export function* addressVotedListFunc(action) {
    try {
        ...
        const data = yield call(iissDelegateList, {
            prep: address
        })
        if (data && data.length > 0) {
            yield put({
                type: AT.addressVotedListFulfilled,
                payload: {
                    data,
                    listSize: data.length,
                    totalSize: data.length
        ...
        yield put({
            type: AT.addressVotedListRejected
        });
...
export function* addressTxListFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.addressTxListFulfilled,
                payload: {
                    data: []
                }
            });
...
export function* addressInternalTxListFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.addressInternalTxListFulfilled,
                payload: {
                    data: []
                }
            });
        ...
        yield put({
            type: AT.addressInternalTxListRejected
        });
    }
}
...
export function* addressTokenTxListFunc(action) {
    try {
        if (action.payload.count === 0) {
            yield put({
                type: AT.addressTokenTxListFulfilled,
                payload: {
                    data: []
                }
            });
        ...
        yield put({
            type: AT.addressTokenTxListRejected
        });
    }
}